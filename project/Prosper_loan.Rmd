...
```{r, echo = FALSE}
loans.df$Prosp_rating_set <-factor(loans$ProsperRating..Alpha.,
...
##Look at Bar-chart for borrower rate by occupation, but for the £25-50k income factor ONLY.
```{r, echo= FALSE}
ggplot(aes(x = Occupation),
       data =  subset(loans.df, IncomeRange =="$25,000-49,999")) +
  geom_bar(stat = "count") +
  coord_flip() +
  ylab('count') +
  xlab('Occupation') +
