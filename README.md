...
- reduced the number of colours from a rainbow palette to just one.
- added introductory paragraph to set the scene better (ok, probably not strictly 'design')
- added the ability to select an occupation and highlight it's data (whilst still keeping the rest of the data plotted for reference)
- Adjusted the speed of the animation, to a level which cycles through the occupations quickly, but not so quick that you cannot see them.
- Identified a mistake in my inital data capture, following feedback (again, not 'design')
- Reduced the weight of the horizontal grid lines to a more muted, less distracting shade of grey.
- NOTE - the previous iterations of the project are linked in the gist. The link to the bl.ocks is http://bl.ocks.org/dcfwight/e8a7e35d8b5c485b844d

Resources
- Udacity Nano course, Mike Bostock, Scott Murray, Nikhil Nathwani blog on building buttons: http://www.nikhil-nathwani.com/blog/posts/radio/radio.html
...
Doug Wight
Udacity's Data Analyst Nanodegree (Discussion)  -  23 Feb 2016

Hi all,
I'd be very grateful for any feedback on my analysis of Prosper Loans.
What I've done is average the loan rate (APR) for each occupation in the Prosper database, and for each income level.

I did some pre-processing of the data in R, to amalgamate some of the occupations, and also to create averages only where there was a minimum of 8 data points.

Have attached the bl.ocks link
Please 'open' the index.html link, so you can see it fully - the initial page does not capture the legend to the right.
You can click on the legend buttons to highlight that occupation only.

As with all others, any feedback is gratefully received - some questions to consider are:
...
Thanks, Doug.﻿
Show less
Average APR by Occupation and income level on Prosper
bl.ocks.org
Doug’s Block 7bd12f0a53cdcdbb9a5a
1John Enyeart's profile photoFang Lu's profile photoDoug Wight's profile photo
Hide comments

John Enyeart23 Feb 2016




I think an introduction paragraph describing what Prosper Loans is and what exactly we're looking at with the data would be helpful.

As far as the visualization itself, you say "I want the user to be able to easily switch between occupations and compare to the average." From what I see, the visualization quickly runs through each occupation and then plots them all together, but there's no way to tell it to stop and just show two occupations of interest, for example. So, some work along those lines would be helpful.﻿
Show less

Doug Wight23 Feb 2016
...
I'll add some introductory text about what exactly the dataset is, to help the viewer understand what's happening. I've been living with this dataset for a while now, so I know it back to front, so I was probably a bit lazy there.

Will also re-size the chart so it fits in the the bl.ocks page - currently it is cut off on the right. This means that the legends are missing. When they appear, it allows the user to select an occupation and compare it to the mean. But because you can't see this without clicking the 'open' link, then the viewer is completely unaware of them.
...
http://bl.ocks.org/dcfwight/e8a7e35d8b5c485b844d﻿
Show less
Add a comment...
